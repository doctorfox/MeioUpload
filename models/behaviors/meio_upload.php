 ...
 * MeioUpload Behavior
 *
 * This behavior is based on Juan Basso's  MeioUpload Behavior
 * (http://www.meiocodigo.com/projects/meioupload/)
 *
 * @author Segun Fodeke (info@humanaware.com)
 ...
 * @Comment      Fixed the validation of file size, file extensions, and mime types
 * @TODO         Show an error message when validation fails
 *               Allow saving to specific model table with on the fly config from each model/controller.
 */
...
class MeioUploadBehavior extends ModelBehavior {
    ...
    function setup(&$model, $settings = array()) {
        //Check for Model to save the file Data
        /*if(empty($settings['model'])){
            $useModel = (isset($settings['useModel']))? $settings['useModel']:$model->alias;//pr($model);
            $model = new $useModel;
        }*/
        ...
        foreach ($settings as $field => $options) {            
            ...
            // Verifies if the thumbsizes names is alphanumeric in order to create valid directory names for the thumbs
            $thumbnames = array_keys($options['thumbsizes']);
            foreach ($thumbnames as $name) {
                if (!ctype_alnum($name)) {
                    trigger_error(__d('meio_upload', "MeioUploadBehavior Error: The thumbsize named: '{$name}' must be alphanumeric.", true), E_USER_ERROR);
                }
            }
    ...
    function uploadMimeType(&$model, $data, $mimeAllowed, $extra = null) {
        ...
        if (!in_array($this->_getMimeType($data['tmp_name'], $data['type']), $mimeAllowed)) {
            return false;
        }
        return true;
    }
    ...
    function uploadExtension(&$model, $data, $extAllowed, $extra = null) {
        ...
        if (!in_array(strtolower(pathinfo($data['name'], PATHINFO_EXTENSION)), $extAllowed)) {
            return false;
        }
        return true;
    }
    ...
    function _uploadCheckSize(&$data, $size, $type) {
        ...
        list($imgWidth, $imgHeight) = getimagesize($data['tmp_name']);
    ...
    function _uploadFile(&$model) {
        ...
        foreach ($this->_config[$model->alias] as $fieldName => $options) {
            ...
            $this->_createFolders($options['dir'], array_keys($options['thumbsizes']), $options['folderPermission']);
            ...
            // If no file has been uploaded, then unset the field to avoid overwriting existent file
            ...
            //Do validation checks based on the behavior config options
            if (isset($options['allowedMime']) && !empty($options['allowedMime'])) {
                $mimecheck = $this->uploadMimeType($model, $data[$model->alias][$fieldName], $options['allowedMime'], true);
                if ($mimecheck !== true) {
                    $return[$fieldName] = $mimecheck;
                    return $return;
                }
            }
            if (isset($options['allowedExt']) && !empty($options['allowedExt'])) {
                $extcheck = $this->uploadExtension($model, $data[$model->alias][$fieldName], $options['allowedExt'], true);
                if ($extcheck !== true) {
                    $return[$fieldName] = $extcheck;
                    return $return;
                }
            }
            if (isset($options['maxSize']) && !empty($options['maxSize'])) {
                $sizecheck = $this->uploadMaxSize($model, $data[$model->alias][$fieldName], $options['maxSize'], true);
                if ($sizecheck !== true) {
                    $return[$fieldName] = $sizecheck;
                    return $return;
                }
            }
    ...
    function _replaceTokens(&$model, $string, $fieldName) {
        return str_replace(
                        array('{ModelName}', '{fieldName}', '{DS}', '/', '\\'), array(Inflector::underscore($model->name), $fieldName, DS, DS, DS), $string
        );
    }
     ...
     * @access protected
     */
    function _createFolders($dir, $thumbsizes, $folderPermission = 0755) {
        $folder = new Folder();

        $folder->create($dir, $folderPermission); // If thumbsizes is empty
        foreach ($thumbsizes as $thumbName) {
            $folder->create($dir . DS . 'thumb' . DS . $thumbName, $folderPermission);
        }
    }
